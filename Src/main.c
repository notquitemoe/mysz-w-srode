...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  /* Start timer with DMA transfer */
  HAL_TIM_Base_Start(&htim3);

  HAL_DMA_Start_IT(&hdma_tim3_up, (uint32_t)&dmaBuffer, (uint32_t)&GPIOC->BSRR, 2);

  /* Enable timer update DMA requests */
...
static void MX_TIM3_Init(void)
{
  ...
  /* Set up timer with 2-second period */
  /* Timer clock = 50MHz (APB1 clock) */
  /* Prescaler = 50000-1, so timer tick is 1ms */
  /* Period = 2000-1, so total period is 2000ms = 2s */
  htim3.Instance = TIM3;
...
static void MX_DMA_Init(void)
{
  ...
  /* Configure DMA for TIM3 update event */
  hdma_tim3_up.Instance = DMA1_Stream2;      /* Using DMA1 Stream2 for TIM3_UP */
  hdma_tim3_up.Init.Channel = DMA_CHANNEL_5; /* Channel 5 for TIM3_UP */
...
{
  /* This callback is not used since DMA handles everything */
}
...
{
  /* This callback is not used since DMA is in circular mode */
}

/* DMA1_Stream2_IRQHandler interrupt handler */
